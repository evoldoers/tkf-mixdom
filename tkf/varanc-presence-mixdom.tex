...
is the empty-domain renormalisation
(equation~\eqref{eq:singlet-end-domain}). Each $\omega^{(r)}$
...
HMM transition kernel of mixdom-wfst.tex
Section~\ref{sec:labeled-singlet}, and let $T^{\text{lab}}_{ss'}$
denote the labelled conditional WFST kernel of
Section~\ref{sec:labeled-wfst}. The labelled per-character Pair HMM
joint factorises as
\begin{equation}
\label{eq:pair-fact}
...
weight; this is the construction of mixdom-wfst.tex
Section~\ref{sec:labeled-wfst} (\textit{Singlet} $\circ$ \textit{WFST}
$=$ \textit{Pair HMM}).

\paragraph{Singlet-table convention.}
Throughout the proof we use the singlet HMM table at
mixdom-wfst.tex line~165--170 with the following normalisation
(consistent with the joint-contribution language at line~187--189
and the marginalisation check at line~228--237). Two structural
...
mixdom-wfst.tex line~228--237 verifies the row sum (over destinations
...
(line~187--189 of mixdom-wfst.tex), independent of $Z_{n-1}$
...
\emph{Proof.} The singlet table mixdom-wfst.tex line~165--170
partitions the source $(g, e)$ values into three rows:
\begin{itemize}[nosep]
...
f')$ factor (mixdom-wfst.tex eq.~\eqref{eq:wfst-weight-principle}
gives the WFST = Pair~HMM/singlet division explicitly). In both
...
$T^{\text{lab}}$. This is by inspection of the labelled WFST tables
of mixdom-wfst.tex Section~\ref{sec:labeled-wfst}.
...
\end{equation}
where $T^{\text{lab}}$ is the \emph{conditional} labelled WFST of
Section~\ref{sec:labeled-wfst}, defined as the labelled Pair HMM
...
transitions do not constrain $e$; the labelled-singlet table at
mixdom-wfst.tex line~165 treats $e$ as a free variable in the
...
factors absorbed --- see Section~\ref{sec:wfst-simplify}).
