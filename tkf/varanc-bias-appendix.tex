...
quadratic-MLE form of the closed-form indel M-step)
...
% The three \ifcsname guards below refer to results sections defined in
% frontmatter-tkf.tex.  tkf.tex inputs that file; the tkf-dp supplement, which
% also inputs this appendix, does not.  The guard renders the cross-reference
% where the label exists and omits it where it does not.
...
(\ifcsname r@sec:results-varanc-vs-fitch\endcsname
Section~\ref{sec:results-varanc-vs-fitch}: \fi VarAnc and Fitch
...
exact-EM (Maraschino\ifcsname r@sec:results-pfam-cherry\endcsname,
Section~\ref{sec:results-pfam-cherry}\fi).
...
parameters\ifcsname r@sec:results-cherry-vs-msa\endcsname{}
(Section~\ref{sec:results-cherry-vs-msa})\fi, where
