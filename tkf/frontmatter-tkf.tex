...
\subsection{Closed-form BDI sufficient-statistic expectations match Gillespie averages}
...
\subsection{TKF91 and TKF92 Forward--Backward sufficient statistics against Gillespie ground truth}
...
$N$, vs.\ truth $0.5$.  One plausible mechanism is that the JC20 (uniform)
substitution model used in this validation benchmark provides
little Match-vs-indel discrimination at high $t$.  To test this
hypothesis, at fixed
$(\insrate, \delrate, \ext) = (0.08, 0.12, 0.5)$ and $N = 1000$ we
...
The actual mechanism, localised by a three-way comparison, turns out
...
equilibrium to within sample noise.
...
produce visible bias on $\hat\ext$; we use the
...
\subsection{TKF92 parameters recovered by Maraschino from Pfam sibling pairs}
...
\subsection{Maraschino alignment-given estimates versus unaligned 2D SVI-BW}
...
\subsection{Ancestral indel-presence reconstruction: VarAnc, Fitch parsimony, and gap-augmented LG08}
...
\paragraph{Interpretation and recommendation.}
...
\subsection{svi-VarAnc refinement on FastTree topologies inflates TKF92 indel rates}
...
\subsubsection{Reconstruction quality: tied versus untied \texorpdfstring{$q$}{q}}
...
($19{,}850$ families), from per-iteration checkpoints.  Cherry-trained
...
\input{varanc-bias-discussion}

\subsection{Cherry-trained versus MSA-refined TKF92 parameters}
...
in Table~\ref{tab:em-convergence-from-truth-inline}.  $^{*}$Values
reported at iteration $85$ of $100$; the inflated direction and
approximate magnitude of the bias are clear and stable by this
point.}
...
In summary, the cherry-trained Maraschino fit and
...
\subsection{Mixture-of-TKF92s fitted to Pfam by EM-around-Maraschino}
...
\subsection{Mixture-of-sites fitted to Pfam by EM-around-CherryML}
...
with no single component dominating.  A Pfam-clan stability check
and an LG-comparison correlation analysis are beyond the scope of
this paper; both are tractable next steps.
...
\subsection{FSA alignment accuracy on BAliBASE and OXBench: TKF92 mixtures versus MAFFT and MUSCLE}
...
$K = 20$ gap).  On BAliBASE, the $K = 20$ mixture is approximately
...
a \emph{posterior dilution} mechanism: $K$-component mixtures spread per-pair posterior mass
...
\subsection{TKF92 as a renormalized surrogate for a general long-indel process}
\label{sec:results-ggi-flow}

TKF92's geometrically distributed fragments are themselves an
approximation to the richer dynamics of a general geometric indel
(GGI) process \cite{Holmes2020}, in which insertions and deletions of
geometric length act at independent rates.  Because the GGI process is
not closed under composition, no single TKF92 reproduces it at every
branch length, and the natural question is how the \emph{best} TKF92
surrogate---the one minimizing the per-residue conditional
Kullback--Leibler divergence to GGI---moves as a function of branch
length $t$.  Composing the TKF92 Pair HMM with an infinitesimal GGI
step and m-projecting the result back onto TKF92 yields a
renormalization-group flow on $(\insrate, \delrate, \ext)$, expressible
as three equivalent ODE systems acting on the BDI sufficient
statistics, the coarse-grained transition counts, or the rate
parameters directly (Appendix~\ref{sec:composition-renorm}).  The
construction follows directly from the automata-theoretic
infinitesimal-generator picture of \cite{Holmes2020}: a GGI generator
acting on the TKF92 alignment distribution.

Along this flow the rates $(\insrate, \delrate)$ drift by about $10\%$
over $t \in [0.05, 10]$ while the fragment-extension probability $\ext$
carries essentially all of the branch-length dependence.  Freezing the
rates at their boundary values reduces the system to a single
$\ext$-ODE with a closed-form first-order solution,
\begin{equation*}
\ext(t) \;\approx\; \ext_\infty + \bigl(\ext(0) - \ext_\infty\bigr)\,
e^{-kt},
\qquad
\ext_\infty = \frac{\ext(0)}{2 - \ext(0)},
\qquad
k = (\insrate(0) + \delrate(0))\,(2 - \ext(0)),
\end{equation*}
whose fixed point and decay constant depend only on the TKF92 boundary
fit.  This first-order $\ext$-flow tracks the full three-parameter ODE
to within $2\%$ for $t \lesssim 1$ and $\sim 10\%$ at $t = 10$, and
follows per-branch maximum-likelihood TKF92 fits to GGI simulations to
within sampling error (Appendix~\ref{sec:composition-renorm},
Figures~\ref{fig:pfam-closedform} and~\ref{fig:pfam-flow-1param}).

The decay constant defines a \emph{fragment-coherence half-life}
$t_{1/2} = \ln 2 / k$, the branch length over which $\ext$ relaxes
halfway to $\ext_\infty$.  At Pfam scale this half-life is large: the
prealigned Maraschino fit ($\insrate \approx \delrate \approx 0.030$,
$\ext \approx 0.65$) gives $t_{1/2} \approx 8.6$, and the unaligned
SVI-BW fit ($\insrate \approx \delrate \approx 0.034$, $\ext \approx
0.58$) gives $t_{1/2} \approx 7$ --- both more than an order of
magnitude above the median Pfam cherry length $t \approx 0.76$.  A
median cherry therefore traverses only a few percent of the available
$\ext$-flow; the $K = 20$ mixture components span $t_{1/2} \in [3.4,
15.0]$ (weighted mean $\approx 9.5$), and even the fastest component
moves only $\sim 14\%$ of its flow over a median cherry.

The practical consequence is that GGI-steering buys little at Pfam
divergences.  Steering $\ext$ along the flow neither significantly
improves nor degrades the TKF92 fit, whether scored on GGI simulations
or on real Pfam cherries, and whether the TKF92 parameters are
estimated from fixed alignments (Maraschino) or alignment-free pairwise
EM (SVI-BW): on GGI-simulated cherries the constant-$\ext$ surrogate
and the GGI-steered surrogate agree in conditional log-likelihood to
within a fraction of a nat per cherry, and on Pfam the large
fragment-coherence half-life leaves the data without the temporal span
needed to resolve the flow.  The flow becomes the dominant correction
only for $t \gtrsim t_{1/2}$ --- faster indel regimes or deeper
branches than Pfam cherries provide --- so constant-$\ext$ TKF92
remains the appropriate default across the phylogenetic range studied
here.
...
\subsection{Mixture results across substitutions, indels, and alignment}
