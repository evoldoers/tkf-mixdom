...
\begin{multline*}
\seq'(\evoltime) \sim
\tkffrags^\nfrag\!\left(\subproc(\exch,\eqm);\insrate,\delrate,
...
MixFrag is a precursor to the MixDom model (Section~\ref{sec:mixdom}): it shares
MixDom's per-fragment latent typing, but retains a single substitution model and
a single, non-nested indel process, and so introduces no null states.
\else
MixFrag is a precursor to the hierarchical MixDom model \cite{LargeHolmes2026}:
it shares MixDom's per-fragment latent typing, but retains a single substitution
model and a single, non-nested indel process, and so introduces no null states.
\fi
...
exactly as for TKF92.
The Pair HMM has no null states, so---unlike the MixDom Pair HMM---no
null-elimination (collapse) step is required.
...
in TKF91.
Because the fragtype is latent and cannot be read off the alignment, the
per-fragtype counts $F_\frag,E_\frag$ are not functions of the observed pairwise
alignment alone, so the Maraschino cherry-count distillation of
Section~\ref{sec:maraschino-intro} does not apply directly; training instead uses
Baum-Welch, or its stochastic variational form.
