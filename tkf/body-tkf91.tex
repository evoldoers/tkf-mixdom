...
\end{eqnarray*}
where the three observable terms count, respectively, the expected number of
ancestral sequence extensions $L$,
ancestral sequence terminations $M$ (always one for a single training alignment),
and positions $V_i$ initially having residue $i$ (including ancestral, descendant, and transiently inserted residues);
$\agecorrection^V_i$ is another genealogical correction term accounting for these transients,
...
Thus $V_i$ counts every observable equilibrium $\eqm$-draw of character $i$ in the trajectory, regardless of whether the residue subsequently underwent a match-position substitution, was deleted, or never had an ancestor (i.e. was an insertion).
