...
\item Conditioned on $i$, the links evolve as independent sub-lineages,
  so $v_\insrate(i) = \Theta(1/i)$. The relative variance \emph{does}
  fall as a pair gets longer, and the Fisher information for $\insrate$
  grows linearly in $i$. What is fixed is not the information per link
  but the equilibrium length, pinned at $\expect[i] = \kappa/(1-\kappa)$.
\end{itemize}
Thus the per-pair Fisher information grows linearly in $i$, as it does
for substitution parameters. Indels differ only in the constant and in
the fact that the stationary length is not a free variable, so at
stationarity extra precision comes from processing more pairs rather
than from lengthening a given one.
