...
$s \sim \eqm(\cdot)$, drawn independently of the attempted destination;
the attempt succeeds (a visible substitution $i \to j$) iff $s = j$, and
is otherwise an invisible rejected attempt. The effective visible rate
$i \to j$ is then $S_{ij}\eqm_j$, as required.

Let $R_i := \sum_{j' \ne i} S_{ij'}$ be the $\eqm$-independent total
exchangeability out of $i$, and set $S_{ii} := 0$. Every attempt carries
exactly one secret destination, so the augmented data are the counts
$N_a$ of attempts whose secret destination is $a$, and, with the
Holmes--Rubin sufficient statistics $\{V_i, U_{ij}, W_i\}$,
\begin{equation}
\label{eq:gtr-secret-dest-cdl}
\ell_{\mathrm{aug}}(\eqm) \;=\; \sum_a N_a \log \eqm_a + \mathrm{const},
\qquad
N_a \;=\; \underbrace{V_a + \textstyle\sum_i U_{ia}}_{=:\,V'_a} \;+\; Z_a .
\end{equation}
An accepted attempt's secret destination \emph{is} its observed
destination, so those tokens are the visible counts $V'_a$; only the
tokens of the rejected attempts, $Z_a$, are latent. The attempt rates
$S_{ij}$ do not involve $\eqm$ and the exit factor
$\exp(-\sum_i W_i R_i)$ is likewise $\eqm$-free, so all of the
$\eqm$-dependence sits in the token counts.

Given the trajectory statistics and the current $\eqm$, the rejected
attempts are a thinned Poisson process: from state $i$, attempts
proposing some $j \ne a$ occur at total rate $R_i - S_{ia}$, and each
carries secret destination $a$ with probability $\eqm_a$. Hence
\begin{equation}
\label{eq:gtr-secret-dest-estep}
\expect[Z_a] \;=\; \eqm_a \sum_i W_i\,(R_i - S_{ia}),
\end{equation}
and maximising \eqref{eq:gtr-secret-dest-cdl} on the simplex gives
$\hat\eqm \propto \expect[N]$, i.e.
\begin{equation}
\label{eq:gtr-secret-dest-eqm}
\boxed{\;
  \hat\eqm_a \;\propto\;
   V_a + \sum_i U_{ia} + \eqm_a \sum_i W_i\,(R_i - S_{ia})
  \;}
\end{equation}
iterated to convergence. The update is a Dirichlet-conjugate count
update given $\{V, U, W\}$, which is also convenient for MCMC on $\eqm$.

\paragraph{The augmented iteration is exact.}
Equations \eqref{eq:gtr-secret-dest-estep}--\eqref{eq:gtr-secret-dest-eqm}
are a genuine E- and M-step for a valid augmentation, so the iteration
increases the true conditional log-likelihood monotonically and its fixed
points are that likelihood's stationary points. Writing
$T_a = \sum_i W_i S_{ia}$ and $G = \sum_i W_i R_i$, a fixed point with
normaliser $Z$ satisfies $\eqm_a Z = V'_a + \eqm_a (G - T_a)$, that is
\begin{equation}
\label{eq:gtr-secret-dest-stationary}
\frac{V'_a}{\eqm_a} - T_a \;=\; Z - G ,
\end{equation}
which is exactly the Lagrange stationarity condition for maximising
$\sum_a V'_a \log \eqm_a - \sum_i W_i \sum_{j \ne i} S_{ij}\eqm_j$ on the
simplex, with the normaliser playing the role of the multiplier. That
objective is strictly concave, so its maximiser is unique and the
iteration recovers it for \emph{any} exchangeability $S$: there is no F81
restriction and no bias from exchangeability heterogeneity.

Two nearby estimators are \emph{not} exact, and the distinction matters.
Dropping the $\eqm_a$ factor from \eqref{eq:gtr-secret-dest-eqm} --- which
would make the augmented counts $\eqm$-independent and turn the update
into a single closed-form step rather than an iteration --- replaces the
E-step by an $\eqm$-independent $q$; the resulting estimator maximises a
lower bound on the likelihood instead of the likelihood, and is biased
for any $S$ that is not off-diagonal-constant. And the naive
$\hat\eqm \propto V'$, which ignores the rejected attempts altogether, is
biased whenever the dwell times $W_a$ are unequal --- including under
F81, where the exact stationary point is
$\eqm_a = V'_a / (\lambda + \mu(W_{\mathrm{tot}} - W_a))$ with
$W_{\mathrm{tot}} = \sum_i W_i$, rather than $\propto V'_a$. The
$\eqm_a$-weighted term in \eqref{eq:gtr-secret-dest-eqm} is precisely the
correction for both. The augmentation is originally due to Bruno~\citep{Bruno1996}.
