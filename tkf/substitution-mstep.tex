...
The trick is exact when $\exch_{ij}$ is constant on the off-diagonal
(the F81 case): $R_i - S_{ij} = (|\alphabet| - 2) \cdot S_{ij}$ is
constant across $j$, the augmentation contributes only a residue-
uniform pseudocount, and \eqref{eq:gtr-secret-dest-eqm} reduces to
Bruno's closed form $\hat\eqm_a \propto V'_a$ (a Dirichlet posterior
mode) \citep{Bruno1996}. Away from F81, \eqref{eq:gtr-secret-dest-eqm}
