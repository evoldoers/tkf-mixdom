#!/usr/bin/env python3
"""Beam weighted reconstruction benchmark for SVI-BW MixDom models.

Adapted from beam_weighted_benchmark.py to accept --model-path argument.
Compares: beam uniform, beam weighted, Felsenstein on Pfam test families.

Usage:
    cd python && JAX_ENABLE_X64=1 CUDA_VISIBLE_DEVICES=0 uv run python \
        experiments/svi_bw_beam_benchmark.py \
        --model-path pfam/svi_bw_d3f1_best_val.npz \
        --out experiments/d3f1_beam.json \
        --n-families 20
"""
...
def main():
    parser = argparse.ArgumentParser(
        description='Beam weighted reconstruction benchmark')
    ...
    n_frag = int(params['frag_weights'].shape[1]) if params['frag_weights'].ndim > 1 else 1
    ...
    log(f'  n_dom={n_dom}, n_frag={n_frag}')

    # Distill for singlet HMM (needed for beam)
        ...
        for held_out in holdouts:
            ...
            try:
                ...
                recon_unif, score_unif = composite_beam_reconstruct_jax(
                    ...
                    beam_width=args.beam_width, max_len=int(len(true_seq) * 1.5),
                    desc_weights=None)
                unif_time = time.time() - tb
                unif_id, _, _ = needleman_wunsch_identity(recon_unif, true_seq)
            except Exception as e:
                log(f'  beam uniform error: {e}')
                unif_id, unif_time = -1.0, 0.0
                recon_unif = np.array([], dtype=np.int32)
            ...
            try:
                ...
                log(f'  beam weighted error: {e}')
                wt_id, wt_time = -1.0, 0.0
                recon_wt = np.array([], dtype=np.int32)
    ...
    # Save
    if results:
        unif_ids = [r['uniform_id'] for r in results if r['uniform_id'] >= 0]
        wt_ids = [r['weighted_id'] for r in results if r['weighted_id'] >= 0]
    ...
    if results:
        output['summary'] = {
            'uniform_mean': float(np.mean([r['uniform_id'] for r in results if r['uniform_id'] >= 0])) if any(r['uniform_id'] >= 0 for r in results) else 0,
            'weighted_mean': float(np.mean([r['weighted_id'] for r in results if r['weighted_id'] >= 0])) if any(r['weighted_id'] >= 0 for r in results) else 0,
            'fels_mean': float(np.mean([r['fels_id'] for r in results if r['fels_id'] >= 0])) if any(r['fels_id'] >= 0 for r in results) else 0,
        }
