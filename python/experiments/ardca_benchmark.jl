...
# reconstruct ancestor, score per-column accuracy vs held-out leaf.
#
# Usage:
#   export PATH="$HOME/.juliaup/bin:$PATH"
#   julia experiments/ardca_benchmark.jl
...
function run_benchmark(;
    split_file  = expanduser("~/bio-datasets/data/pfam-seed/splits/v1.json"),
    ...
    min_cols    = 50,
    max_cols    = 200,
    output_file = joinpath(@__DIR__, "ardca_benchmark_results.json"),
)
    splits = JSON.parsefile(split_file)
    val_families = splits["val"]
    println("Loaded $(length(val_families)) val families")
    ...
    for (idx, family) in enumerate(val_families)
        n_processed >= max_families && break
        ...
        (nseqs < min_seqs || nseqs > max_seqs) && (n_skipped += 1; continue)
        ...
        (N_sites < min_cols || N_sites > max_cols) && (n_skipped += 1; continue)

        println("\n[$idx] $family: $nseqs seqs, $N_sites cols")

        try
            # Hold out first leaf
            holdout_idx = 1
            holdout_name = seq_names[holdout_idx]
            ...
            # Write remaining to temp FASTA and fit ArDCA
            tmpdir = mktempdir()
            fasta_file = joinpath(tmpdir, "remain.fasta")
            write_fasta(fasta_file, remain_names, Z_remain)

            println("  Fitting ArDCA...")
            arnet, arvar = ardca(fasta_file; verbose=false)

            # Build NJ tree from ALL sequences (full topology)
            # Use sanitized names for Newick compatibility
            println("  Building NJ tree...")
            san_names = [sanitize_newick_label(n) for n in seq_names]
            san_holdout = sanitize_newick_label(holdout_name)
            san_remain = [sanitize_newick_label(n) for n in remain_names]
            # Map sanitized -> original index
            san_to_idx = Dict(san_names[i] => i for i in 1:nseqs)

            D = pairwise_distances(Z_full)
            newick_str = neighbor_joining(D, seq_names)  # NJ already sanitizes
            ...
            # ArDCA may infer q=20 if no gaps in the training data; skip those.
            q = length(arnet.p0)
            if q != 21
                println("  Skip: ArDCA q=$q (expected 21 with gaps)")
                n_skipped += 1
            ...
            # Prepare leaf sequences (ASR encoding) with sanitized names
