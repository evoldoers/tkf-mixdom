#!/usr/bin/env python3
"""FSA alignment benchmark on BAliBASE using a MixFrag pair HMM.

Mirrors ``fsa_tkf92_balibase.py`` but swaps the single-TKF92 pair HMM for the
MixFrag (TKF92 fragment-mixture) pair HMM via ``pairwise_posteriors_mixfrag``.
Identical FSA pipeline: per-pair branch length tau is estimated by
Newton-Raphson on E[LL(tau)] (reduced from one forward-backward), the
forward-backward is re-run at tau_opt, and the resulting match-state posteriors
(summed over all M_f fragtype states) drive sequence annealing.

LG08 substitution emissions; MixFrag indel + fragtype params (lam, mu, {r_f},
{w_f}) loaded from an ``.npz`` produced by ``train_mixfrag_cherry_em.py`` --
default ``params/best/cem_mixfrag_F2_pfamTrain.npz`` (the F=2 fit on the Pfam
v1 train split). At F=1 this reproduces ``fsa_tkf92_balibase.py`` exactly.

Defaults to GPU. Override with JAX_PLATFORMS=cpu / CUDA_VISIBLE_DEVICES=
to force CPU.

Usage:
    cd python && JAX_ENABLE_X64=1 \
        uv run python experiments/fsa_mixfrag_balibase.py \
        --params-npz params/best/cem_mixfrag_F2_pfamTrain.npz \
        --out experiments/balibase_mixfrag_F2.json
"""

import os
os.environ.setdefault('JAX_ENABLE_X64', '1')

import json
import sys
import time
from pathlib import Path

import numpy as np
import jax.numpy as jnp

sys.path.insert(0, str(Path(__file__).parent.parent))

from tkfmixdom.jax.tree.fsa_anneal import (
    _pairwise_posteriors_mixfrag_jax,
    select_pairs_full, select_pairs_erdos_renyi,
)
from tkfmixdom.jax.dp.hmm import _pad_to_bin, _pad_seq
from tkfmixdom.jax.core.protein import rate_matrix_lg
from tkfmixdom.util.msa_benchmark import parse_fasta, sp_tc_score
from tkfmixdom.util.expected_pair_f1 import expected_family_f1

# Reuse helpers from the OXBench script (encoding, MSA-from-posteriors,
# msa-to-strings) -- same as fsa_tkf92_balibase.py.
from fsa_mixdom_pairhmm_oxbench import (
    encode_seq, build_msa_from_posteriors,
    build_msa_from_posteriors_multi, msa_to_aligned_strings,
    AA_CHARS, AA_MAP,
)


def _run_pair_loop(int_seqs, names, pairs, ins, del_, exts, weights, Q, pi):
    """Per-pair MixFrag posterior computation for a fixed pair list."""
    pair_posteriors = {}
    last_tau = 0.0
    exts_j = jnp.asarray(exts)
    weights_j = jnp.asarray(weights)
    for i_idx, j_idx in pairs:
        x = jnp.asarray(int_seqs[names[i_idx]], dtype=jnp.int32)
        y = jnp.asarray(int_seqs[names[j_idx]], dtype=jnp.int32)
        Lx_pad = _pad_to_bin(int(x.shape[0]))
        Ly_pad = _pad_to_bin(int(y.shape[0]))
        x_pad = _pad_seq(x, Lx_pad)
        y_pad = _pad_seq(y, Ly_pad)
        real_Lx = int(x.shape[0])
        real_Ly = int(y.shape[0])
        mp_pad, tau, lp = _pairwise_posteriors_mixfrag_jax(
            x_pad, y_pad,
            jnp.int32(real_Lx), jnp.int32(real_Ly),
            jnp.float64(ins), jnp.float64(del_), exts_j, weights_j,
            jnp.asarray(Q), jnp.asarray(pi))
        mp = np.asarray(mp_pad)[:real_Lx, :real_Ly]
        pair_posteriors[(i_idx, j_idx)] = mp
        last_tau = float(tau)
    return pair_posteriors, last_tau


def process_family(family_name, in_dir, ref_dir, ins, del_, exts, weights, Q, pi,
                   full_pair_cutoff=30, n_seeds=1, seed_base=42):
    """Process one BAliBASE family with the MixFrag pair HMM.

    Pair selection is automatic (``full`` if ``n_seqs <= full_pair_cutoff``,
    else ``erdos_renyi``; OOM on full falls back to erdos_renyi).
    """
    import gc
    import jax
    in_path = os.path.join(in_dir, family_name)
    ref_path = os.path.join(ref_dir, family_name)

    if not os.path.exists(in_path) or not os.path.exists(ref_path):
        return None

    raw_seqs = parse_fasta(in_path)
    if len(raw_seqs) < 2:
        return None

    int_seqs = {}
    for name, seq in raw_seqs.items():
        enc = encode_seq(seq)
        if len(enc) == 0:
            continue
        int_seqs[name] = enc
    if len(int_seqs) < 2:
        return None

    names = list(int_seqs.keys())
    n_seqs = len(names)

    use_full = n_seqs <= full_pair_cutoff
    pairs = select_pairs_full(n_seqs) if use_full else \
        select_pairs_erdos_renyi(n_seqs)
    selection_used = 'full' if use_full else 'erdos_renyi'

    t0 = time.time()
    try:
        pair_posteriors, last_tau = _run_pair_loop(
            int_seqs, names, pairs, ins, del_, exts, weights, Q, pi)
    except jax.errors.JaxRuntimeError as e:
        if 'RESOURCE_EXHAUSTED' not in str(e) or selection_used == 'erdos_renyi':
            raise
        print(f"    [OOM-fallback] {family_name}: full ({len(pairs)} pairs) "
              f"OOM'd; retrying with erdos_renyi", flush=True)
        jax.clear_caches()
        gc.collect()
        pairs = select_pairs_erdos_renyi(n_seqs)
        selection_used = 'erdos_renyi'
        pair_posteriors, last_tau = _run_pair_loop(
            int_seqs, names, pairs, ins, del_, exts, weights, Q, pi)

    ref_aln = parse_fasta(ref_path)

    expected_micro = expected_family_f1(
        pair_posteriors, ref_aln, names, core_only=True)['micro']

    if n_seeds <= 1:
        msa_dict, msa_length = build_msa_from_posteriors(
            int_seqs, pair_posteriors, seed=seed_base)
        aligned = msa_to_aligned_strings(msa_dict)
        t_elapsed = time.time() - t0
        sp, tc = sp_tc_score(aligned, ref_aln, core_only=True)
        return {
            'family': family_name,
            'n_seqs': int(n_seqs),
            'n_pairs': int(len(pairs)),
            'pair_selection': selection_used,
            'msa_length': int(msa_length),
            'sp': float(sp),
            'tc': float(tc),
            'expected_f1_micro': expected_micro,
            'time': float(t_elapsed),
            'tau_mean': float(last_tau),
            'n_seeds': 1,
        }

    seeds = [seed_base + k for k in range(n_seeds)]
    runs = build_msa_from_posteriors_multi(
        int_seqs, pair_posteriors, seeds, n_anneal=3)
    per_seed = []
    for seed, msa_dict, msa_length in runs:
        aligned = msa_to_aligned_strings(msa_dict)
        sp, tc = sp_tc_score(aligned, ref_aln, core_only=True)
        per_seed.append({'seed': seed, 'sp': float(sp), 'tc': float(tc),
                         'msa_length': int(msa_length)})
    t_elapsed = time.time() - t0
    sp_arr = np.array([r['sp'] for r in per_seed])
    tc_arr = np.array([r['tc'] for r in per_seed])
    best_idx = int(np.argmax(sp_arr + tc_arr))
    return {
        'family': family_name,
        'n_seqs': int(n_seqs),
        'n_pairs': int(len(pairs)),
        'pair_selection': selection_used,
        'msa_length': int(per_seed[best_idx]['msa_length']),
        'sp': float(per_seed[best_idx]['sp']),
        'tc': float(per_seed[best_idx]['tc']),
        'sp_mean': float(sp_arr.mean()), 'sp_std': float(sp_arr.std()),
        'tc_mean': float(tc_arr.mean()), 'tc_std': float(tc_arr.std()),
        'per_seed': per_seed,
        'best_seed': int(per_seed[best_idx]['seed']),
        'expected_f1_micro': expected_micro,
        'time': float(t_elapsed),
        'tau_mean': float(last_tau),
        'n_seeds': int(n_seeds),
    }


def _load_mixfrag_params(path):
    """Load (lam, mu, exts, weights) from a train_mixfrag_cherry_em .npz."""
    z = np.load(path)
    return (float(z['lam']), float(z['mu']),
            np.asarray(z['exts'], np.float64), np.asarray(z['weights'], np.float64))


def main():
    import argparse
    parser = argparse.ArgumentParser(
        description='FSA alignment benchmark on BAliBASE with MixFrag')
    parser.add_argument('--n-families', type=int, default=0,
                        help='Number of families (0=all, default: all)')
    parser.add_argument('--params-npz', type=str,
                        default='params/best/cem_mixfrag_F2_pfamTrain.npz',
                        help='Path to a train_mixfrag_cherry_em .npz fit.')
    parser.add_argument('--out', type=str,
                        default='experiments/balibase_mixfrag_F2.json')
    parser.add_argument('--full-pair-cutoff', type=int, default=30)
    parser.add_argument('--label', type=str, default='mixfrag_F2_lg08')
    parser.add_argument('--families', type=str, default=None,
                        help='Comma-separated explicit family list.')
    parser.add_argument('--n-seeds', type=int, default=1)
    parser.add_argument('--seed-base', type=int, default=42)
    args = parser.parse_args()

    ins, del_, exts, weights = _load_mixfrag_params(args.params_npz)
    F = len(exts)
    print(f'Loaded MixFrag params from {args.params_npz}:')
    print(f'  lam={ins:.5f}, mu={del_:.5f}, kappa={ins/del_:.4f}, F={F}')
    print(f'  exts={np.array2string(exts, precision=4)} '
          f'weights={np.array2string(weights, precision=4)}')

    Q_lg, pi_lg = rate_matrix_lg()
    Q_lg, pi_lg = np.asarray(Q_lg), np.asarray(pi_lg)
    print(f'  emissions: LG08 ({pi_lg.shape[0]}x{pi_lg.shape[0]} GTR)')

    balibase_dir = Path("~/bio-datasets/data/balibase/bali3pdbm").expanduser()
    in_dir = str(balibase_dir / "in")
    ref_dir = str(balibase_dir / "ref")

    if args.families:
        families = [f.strip() for f in args.families.split(',') if f.strip()]
    else:
        families = sorted(os.listdir(in_dir))
        families = [f for f in families if os.path.exists(os.path.join(ref_dir, f))]
        if args.n_families > 0:
            families = families[:args.n_families]

    print(f'\nProcessing {len(families)} BAliBASE families '
          f'(full-pair-cutoff={args.full_pair_cutoff})')
    print(f'{"Family":<12} {"N":>2} | {"MixFrag SP":>10} {"TC":>6} | {"time":>5}')
    print('-' * 57)

    results = []
    for fi, family in enumerate(families):
        raw_seqs = parse_fasta(os.path.join(in_dir, family))
        n_seq = len(raw_seqs)
        line = f'{family:<12} {n_seq:>2} | '
        try:
            res = process_family(family, in_dir, ref_dir, ins, del_, exts, weights,
                                 Q_lg, pi_lg, full_pair_cutoff=args.full_pair_cutoff,
                                 n_seeds=args.n_seeds, seed_base=args.seed_base)
            if res is not None:
                results.append(res)
                line += f'{res["sp"]:>10.3f} {res["tc"]:>6.3f} | {res["time"]:>5.1f}s'
            else:
                line += f'{"SKIP":>10} {"":>6} |'
        except Exception as e:
            line += f'{"ERR":>10} {"":>6} | {e}'
            import traceback; traceback.print_exc()
        print(f'[{fi+1:>3}/{len(families)}] {line}', flush=True)

    def _corpus_micro_f1(res_list):
        """Corpus-micro presence-F1 = 2 Sum(e_tp)/(Sum(total_mass)+Sum(gold))
        over all families (the project's headline metric; pools components,
        not a mean of per-family F1)."""
        e_tp = sum(r['expected_f1_micro']['e_tp'] for r in res_list
                   if r.get('expected_f1_micro'))
        tm = sum(r['expected_f1_micro']['total_mass'] for r in res_list
                 if r.get('expected_f1_micro'))
        gold = sum(r['expected_f1_micro']['gold'] for r in res_list
                   if r.get('expected_f1_micro'))
        prec = e_tp / tm if tm > 0 else 0.0
        rec = e_tp / gold if gold > 0 else 0.0
        f1 = 2 * e_tp / (tm + gold) if (tm + gold) > 0 else 0.0
        return {'precision': prec, 'recall': rec, 'f1': f1,
                'e_tp': e_tp, 'total_mass': tm, 'gold': gold}

    print('\n' + '=' * 50)
    micro = _corpus_micro_f1(results) if results else {}
    if results:
        sps = [r['sp'] for r in results]; tcs = [r['tc'] for r in results]
        times = [r['time'] for r in results]
        print(f'MixFrag F={F} (n={len(results)}):')
        print(f'  SP: mean={np.mean(sps):.4f}, median={np.median(sps):.4f}')
        print(f'  TC: mean={np.mean(tcs):.4f}, median={np.median(tcs):.4f}')
        print(f'  expected corpus-micro F1={micro["f1"]:.4f} '
              f'(P={micro["precision"]:.4f}, R={micro["recall"]:.4f})')
        print(f'  Time: mean={np.mean(times):.1f}s, total={np.sum(times):.0f}s')

    output = {
        'benchmark': 'BAliBASE 3 (bali3pdbm)',
        'model': f'MixFrag F={F} pair HMM (LG08 emissions)',
        'mixfrag_params': {'lam': ins, 'mu': del_,
                           'exts': exts.tolist(), 'weights': weights.tolist()},
        'params_npz': args.params_npz,
        'n_families': len(results),
        'full_pair_cutoff': args.full_pair_cutoff,
        'label': args.label,
        'results': results,
        'summary': {
            'sp_mean': float(np.mean([r['sp'] for r in results])) if results else 0,
            'sp_median': float(np.median([r['sp'] for r in results])) if results else 0,
            'tc_mean': float(np.mean([r['tc'] for r in results])) if results else 0,
            'tc_median': float(np.median([r['tc'] for r in results])) if results else 0,
            'expected_micro_f1': micro,
            'time_total': float(np.sum([r['time'] for r in results])) if results else 0,
        },
    }
    with open(args.out, 'w') as f:
        json.dump(output, f, indent=2)
    print(f'\nResults saved to {args.out}')


if __name__ == '__main__':
    main()
